...
namespace Microsoft.AspNetCore.Sockets.Internal
{
    public static class SocketLoggerExtensions
    {
        ...
        private static readonly Action<ILogger, DateTime, int, string, Exception> _writingMessage =
            LoggerMessage.Define<DateTime, int, string>(LogLevel.Debug, 1, "{time}: Writing a {count} byte message to request {requestId}.");

        private static readonly Action<ILogger, DateTime, string, Exception> _longPollingDisconnected =
            LoggerMessage.Define<DateTime, string>(LogLevel.Debug, 2, "{time}: Client disconnected from Long Polling endpoint for request {requestId}.");

        private static readonly Action<ILogger, DateTime, string, Exception> _longPollingTerminated =
            LoggerMessage.Define<DateTime, string>(LogLevel.Error, 3, "{time}: Long Polling transport was terminated due to an error on request {requestId}.");
